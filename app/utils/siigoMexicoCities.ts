// Generado del catálogo oficial "Países, estados y ciudades / municipios" de Siigo México.
// https://saprodcentralassets.blob.core.windows.net/siigoapi/documentation/Paises-Estados-Ciudades.xlsx
type SiigoMexicoCityEntry = [value: string, name: string]

const SIIGO_MEXICO_CITIES_BY_STATE: Record<string, SiigoMexicoCityEntry[]> = {
  1: [
    ['1', 'Aguascalientes'],
    ['2', 'Asientos'],
    ['3', 'Calvillo'],
...
export function siigoMexicoCityOptions(stateCode: string) {
  return (SIIGO_MEXICO_CITIES_BY_STATE[stateCode] || []).map(([value, name]) => ({
    value,
    label: `${value} · ${name}`
  }))
}

export function normalizeSiigoMexicoCityCode(stateCode: string, cityCode: string | undefined) {
  const normalized = cityCode?.trim().replace(/^0+(?=\d)/, '') || ''
  return (SIIGO_MEXICO_CITIES_BY_STATE[stateCode] || []).some(([value]) => value === normalized)
    ? normalized
    : ''
}
