...
export default eventHandler(async (event): Promise<ExpenseRecord> => {
  ...
  const existing = id ? await usePrisma().expense.findUnique({ where: { id } }) : null
  if (!existing) {
    throw createError({ statusCode: 404, statusMessage: 'El gasto no existe.' })
  }
  if (!canViewExpenseCategory(user.role, existing.category)) {
    throw createError({ statusCode: 403, statusMessage: 'No tienes permiso para editar este gasto.' })
  }
  ...
  if (!canViewExpenseCategory(user.role, parsed.data.category)) {
    throw createError({
      statusCode: 403,
      statusMessage: 'No tienes permiso para editar gastos de esta categoría.'
    })
  }
