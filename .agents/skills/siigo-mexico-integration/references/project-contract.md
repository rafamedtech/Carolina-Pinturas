...
- Cuando el pedido ya tiene una factura PPD asociada, cada pago local muestra una acción explícita e individual para crear una recepción en Siigo mediante `/v1/vouchers`; no se agrupan pagos. La decisión inicial del 2026-08-28 de dejarla en borrador fue sustituida el 2026-09-03: la acción confirmada crea y timbra la recepción inmediatamente, usando el correo requerido por Siigo.
...
- El 2026-09-03 se observó que `GET /v1/invoices/{id}` puede devolver `balance: 0` para una factura PPD sin recepciones. Para registrar pagos, aceptar tanto `payment` como `payments`, usar el total de sus condiciones o, si Siigo las omite, el total de la factura como techo; restar siempre las recepciones ya vinculadas localmente a la misma factura.
