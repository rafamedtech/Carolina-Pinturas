...
- El 2026-09-03 se observó que `GET /v1/invoices/{id}` puede devolver `balance: 0` para una factura PPD sin recepciones, aun cuando `payment.conditions[].value` conserva el importe pendiente. Para registrar pagos, usar esa condición como límite cuando el saldo sea cero y restar las recepciones ya vinculadas localmente a la misma factura.
